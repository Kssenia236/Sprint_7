...
class TestLoginCourier:
    ...
    def test_courier_login_invalid_data(self, base_url, courier_data, create_courier):
        ...
        test_cases = [
            {"login": "incorrect_" + courier_data["login"], "password": courier_data["password"],
             "expected_status": 404, "expected_message": "Учетная запись не найдена"},
            {"login": courier_data["login"], "password": "incorrect_" + courier_data["password"],
             "expected_status": 404, "expected_message": "Учетная запись не найдена"}
        ]

        for case in test_cases:
            with allure.step(f"Проверка с логином '{case['login']}' и паролем '{case['password']}'"):
                login_data = {"login": case["login"], "password": case["password"]}
                response = requests.post(f"{base_url}/login", json=login_data)
                assert response.status_code == case["expected_status"]
                response_data = response.json()
                assert response_data["message"] == case["expected_message"]

    @allure.title("Проверка авторизации курьера с отсутствующими обязательными полями")
    @pytest.mark.parametrize("request_data",
                             [{"login": "", "password": "test_password"}, {"password": "test_password"}])
